...
std::unordered_set<StrictRelations::Variable*> report_requiredSets; //report

Primitives StrictRelations::P;


STATISTIC(NumVariablesConst, "BASE - Number of variables in constraints");
STATISTIC(NumConstraints, "BASE - Number of constraints");
STATISTIC(NumResolve, "BASE - Number of resolve operations");
STATISTIC(NumNodes, "BASE - Number of dep graph nodes");
STATISTIC(NumEdges, "BASE - Number of dep graph edges");
STATISTIC(NumQueries, "BASE - Number of alias queries received");
STATISTIC(NumNoAlias, "BASE - Number of NoAlias answers");
//STATISTIC(NumNoAlias1, "BASE - Number of NoAlias answers in test 1");
STATISTIC(NumNoAlias2, "BASE - Number of NoAlias answers in test 2");
//STATISTIC(NumNoAlias3, "BASE - Number of NoAlias answers in test 3");
STATISTIC(NumNoAliasGep, "BASE - Number of NoAlias answers in test GEP");
STATISTIC(Nu, "BASE - Number of NoAlias answers in test 2 SRAA");
//STATISTIC(NumEvil, "BASE - Number of evil things that happened");
//STATISTIC(NumProcessedSets, "REPORT - BASE - Number of processed LT sets total");
//STATISTIC(NumProcessedSets_ne, "REPORT - BASE - Number of processed LT sets non empty");
//STATISTIC(NumRequiredSets, "REPORT - BASE - Number of required LT sets");
//STATISTIC(NumRequiredSets_ne, "REPORT - BASE - Number of required LT sets non empty");
...
StrictRelations::~StrictRelations() {
  Nu = NumNoAlias2 - NumNoAliasGep;
  phases += test1 + test2 + test3;
  errs() << "------------------------------------------\n";
  errs() << "                Times                     \n";
  errs() << "------------------------------------------\n";
//  errs() << "BASE - Total time: " << phases << "\n";
  errs() << "BASE - Constraint collection time: " << phase1 << "\n";
  errs() << "BASE - Dependence graph time: " << phase2 << "\n";
  errs() << "BASE - Worklist time: " << phase3 << "\n";
  //errs() << "BASE - Module visit (base) : " << phase5 << "\n";
  //errs() << "BASE - Test 1 time: " << test1 << "\n";
  errs() << "BASE - Test 2 time: " << test2 << "\n";
 // errs() << "BASE - Test ind: " << testind << "\n";
  //errs() << "BASE - Test 3 time: " << test3 << "\n";
  //errs() << "BASE - Test 5 (base) time: " << test5 << "\n";
  errs() << "BASE - Total Worklist (2)  : " << phase1 + phase2 + phase3 + test2 << "\n";
  errs() << "BASE - Constraint Solving of Total  : " << (phase3*100) / (phase1 + phase2 + phase3 + test2) << "\n";
  //errs() << "BASE - Total base time (2) : " << phase5 + test5 << "\n";
  errs() << "------------------------------------------\n";
...
bool StrictRelations::runOnModule(Module &M) {
  ...
  test1 = 0; test2 = 0; test3 = 0; test5 = 0; testind = 0;
...
void LT::resolve() const {
  ...
  // ==================================== //
*/
  DEBUG_WITH_TYPE("worklist", left->printStrictRelations(errs())); ////remover
  DEBUG_WITH_TYPE("worklist", right->printStrictRelations(errs())); ////remover
  DEBUG_WITH_TYPE("worklist", errs() << "\n----\n"); ////remover
  // x < y
  StrictRelations::VariableSet changed;
