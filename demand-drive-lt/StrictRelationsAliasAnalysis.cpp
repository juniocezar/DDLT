...
STATISTIC(NumVariablesConst, "OD - Number of variables in constraints");
STATISTIC(NumConstraints, "OD - Number of constraints");
STATISTIC(NumResolve, "OD - Number of resolve operations");
STATISTIC(NumNodes, "OD - Number of dep graph nodes");
STATISTIC(NumEdges, "OD - Number of dep graph edges");
STATISTIC(NumQueries, "OD - Number of alias queries received");
STATISTIC(NumNoAlias, "OD - Number of NoAlias answers");
//STATISTIC(NumNoAlias1, "OD - Number of NoAlias answers in test 1");
//STATISTIC(NumNoAlias2, "OD - Number of NoAlias answers in test 2");
//STATISTIC(NumNoAlias3, "OD - Number of NoAlias answers in test 3");
STATISTIC(NumNoAlias4, "OD - Number of NoAlias answers in test 4");
STATISTIC(NumNoAliasGep, "OD - Number of NoAlias answers in test GEP");
STATISTIC(Nu, "OD - Number of NoAlias answers in test 4 SRAA");
//STATISTIC(NumEvil, "OD - Number of evil things that happened");
//STATISTIC(NumProcessedSets, "REPORT - OD - Number of processed LT sets");
//STATISTIC(NumProcessedSets_ne, "REPORT - OD - Number of processed LT sets non empty");
//STATISTIC(NumRequiredSets, "REPORT - OD - Number of required LT sets");
//STATISTIC(NumRequiredSets_ne, "REPORT - OD - Number of required LT sets non empty");

uint32_t Constraint::regIds = 1; // Initial id to be associated to a region
...
StrictRelations::~StrictRelations() {
  Nu = NumNoAlias4 - NumNoAliasGep;
  phases += test1 + test2 + test3;
  errs() << "------------------------------------------\n";
  errs() << "                Times                     \n";
  errs() << "------------------------------------------\n";
  //errs() << "Total time: " << phases << "\n";
  errs() << "OD - Constraint collection time : " << phase1 << "\n";
  errs() << "OD - Dependence graph time      : " << phase2 << "\n";
//  errs() << "OD - Worklist time              : " << phase3 << "\n";
  //errs() << "OD - Base over Module           : " << phase5 << "\n\n";
  //errs() << "OD - Test 1 time       : " << test1 << "\n";
  //errs() << "OD - Test 2 time       : " << test2 << "\n";  
  //errs() << "OD - Test 3 time       : " << test3 << "\n";
  errs() << "OD - Test 4 time (on)  : " << test4 << "\n";
  //errs() << "OD - Test base (no-aa) : " << test5 << "\n\n";
  //errs() << "Total WL   : " << phase1 + phase3 + test2 << "\n";
  errs() << "OD - Total WL (on): " << phase1 + phase2 + test4 << "\n";
  //errs() << "OD - Total base : " << phase5 + test5 << "\n";
  errs() << "------------------------------------------\n";

/*  for (const auto &var : variables) {
    if(var.second->changed) NumProcessedSets++;
    if(var.second->changed and not var.second->LT.empty()) NumProcessedSets_ne++;
  }
*/
...
StrictRelations::alias(const MemoryLocation &LocA, const MemoryLocation &LocB) {
  ...
  bool t4 = aliastest4(p1, p2);  
  
  
  if(t4) { 
    t = clock() - t;
    test4 += ((float)t)/CLOCKS_PER_SEC;
    NumNoAlias4++; 
    NumNoAlias++; 
    return NoAlias;
  }
  ...
  return MayAlias; //AliasAnalysis::alias(LocA, LocB);   
}

bool StrictRelations::aliastest4(const Value* p1, const Value* p2) {
  
  if(nodes.count(p1) and nodes.count(p2)){ //Required   
    if(nodes[p1]->mustalias == nodes[p2]->mustalias){
      return false; //return MustAlias;
    }
  }
  ...
  unsigned int id1 = v1->id;
  unsigned int id2 = v2->id;

/*  if(not report_requiredSets.count(v1)) {
    report_requiredSets.insert(v1);
    NumRequiredSets++;
    if(not v1->LT.empty()) NumRequiredSets_ne++;
  }

  if(not report_requiredSets.count(v2)) {
    report_requiredSets.insert(v2);
    NumRequiredSets++;
    if(not v2->LT.empty()) NumRequiredSets_ne++;
  }

  */

  if(id1 == id2 or v1->eqReg.count(id2)){
      if(not v1->solved){        
        wle->onDemand(v1);
      }
      if(not v2->solved){        
  ...
  if(const GetElementPtrInst* gep1 = dyn_cast<GetElementPtrInst>(p1))
    if(const GetElementPtrInst* gep2 = dyn_cast<GetElementPtrInst>(p2)) {
      if(nodes.count(gep1->getPointerOperand()) and nodes.count(gep2->getPointerOperand()))
...
bool StrictRelations::runOnModule(Module &M) {
  ...
  test1 = 0; test2 = 0; test3 = 0; test5 = 0; test4 = 0;

/*  NumProcessedSets = 0;
  NumProcessedSets_ne = 0;
  NumRequiredSets = 0;
  NumRequiredSets_ne = 0;
*/
...
*/
        ...
        // Sigma
          ...
          if(!isa<BranchInst>(cmpBB->getTerminator())) {
            DEBUG(errs() << "Error on evaluating sigma!\n");
            continue;
          }
...
void WorkListEngine::onDemand(StrictRelations::Variable* v) {
  //for(auto i : var_to_const[v]) push(i);
  unsigned int id = v->id;
  if(not processedRegion.count(id)){
    std::vector<Constraint*> vet = constrInReg[id];
    for(auto i: vet){
      push(i);
    }
    processedRegion.insert(id);
  }

  for(auto i : v->constraints){
    push(i);
  }
...
void WorkListEngine::add(const Constraint* C) {
  //var_to_const[C->getLeftSide()].insert(C);
...
void insertLT(StrictRelations::Variable* x,
  ...
  /*
  // ============ REPORT ================ //
  // This report must be disabled while 
  // measuring runtime  
  if(!x->LT.count(y) and x != y){
    if(not report_processedSets.count(x->v)){
      report_processedSets.insert(x->v);
      NumProcessedSets++;
    }  
  }
  // ==================================== //
...
void insertGT(StrictRelations::Variable* x,
  ...
  // This report must be disabled while 
  // measuring runtime  
  if((!x->GT.count(y) and x != y) and (!y->LT.count(x))) {
    if(not report_processedSets.count(y->v)){
      report_processedSets.insert(y->v);
      NumProcessedSets++;
    }  
  }
  // ==================================== //
...
void LT::resolve() const {
  ...
  // This report must be disabled while 
  // measuring runtime  
    if(not report_processedSets.count(right->v)){
      report_processedSets.insert(right->v);
      NumProcessedSets++;
    }  
  // ==================================== //
  */
...
void REQ::resolve() const { 
  ...
  // This report must be disabled while 
  // measuring runtime  
    if(not report_processedSets.count(left->v)){
      report_processedSets.insert(left->v);
      NumProcessedSets++;
    }  
  // ==================================== //
  */
...
void PHI::resolve() const { 
  ...
  // measuring runtime  
    ...
    for(auto i : UGT) {
      if(not report_processedSets.count(i->v)){
        report_processedSets.insert(i->v);
        NumProcessedSets++;
      }
    }
